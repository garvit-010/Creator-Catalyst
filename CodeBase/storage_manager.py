...
from database import (
    get_database, Database, Video, ContentOutput, GroundingReport
)
...
class StorageManager:
    ...
    def save_analysis_results(
        ...
        if results.get('blog_post'):
            self._save_blog_post(
                video_id, 
                results['blog_post'],
                results.get('blog_post_original')
            )
    ...
    def _save_blog_post(self, video_id: int, blog_post: str, original: Optional[str] = None):
    ...
    def _save_grounding_report(self, video_id: int, grounding_metadata: Dict):
        ...
        report = GroundingReport(
            ...
            shorts_verification_rate=grounding_metadata.get('shorts_verification_rate', 0.0),
    ...
    def get_all_videos_summary(self, limit: int = 100) -> List[Dict]:
        ...
        for video in videos:
            ...
            for content in contents:
                content_counts[content.content_type] = content_counts.get(content.content_type, 0) + 1
    ...
    def import_video_results(self, json_path: str) -> int:
        ...
        if data.get('blog_post'):
            self._save_blog_post(video_id, data['blog_post'], data.get('blog_post_original'))
        ...
        if data.get('thumbnail_ideas'):
            ideas = [item['idea'] if isinstance(item, dict) else item for item in data['thumbnail_ideas']]
