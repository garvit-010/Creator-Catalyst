...
def render_credits_page(credits_manager):
    ...
    tabs = st.tabs(
        ["💰 Purchase Credits", "📊 Usage Statistics", "📜 Transaction History"]
    )
    ...
    with tabs[0]:
        ...
        with col1:
            with st.container(border=True):
                ...
                if st.button(
                    "Purchase Starter", key="buy_starter", use_container_width=True
                ):
                    # Simulate purchase (in production, integrate payment gateway)
                    new_balance = credits.add_credits(
                        50, description="Purchased Starter Package ($9.99)"
                    )
        ...
        with col2:
            with st.container(border=True):
                ...
                if st.button(
                    ...
                    new_balance = credits.add_credits(
                        150, description="Purchased Pro Package ($24.99)"
                    )
        ...
        with col3:
            with st.container(border=True):
                ...
                if st.button(
                    "Purchase Business", key="buy_business", use_container_width=True
                ):
                    new_balance = credits.add_credits(
                        500, description="Purchased Business Package ($79.99)"
                    )
        ...
        with st.expander("🔧 Admin: Add Custom Credits"):
            ...
            with col1:
                custom_amount = st.number_input(
                    "Amount to add", min_value=1, max_value=10000, value=10, step=1
                )
        ...
        if not operation_counts:
            ...
            with col1:
                if stats["total_spent"] > 0:
                    video_uploads = operation_counts.get("video_upload", {}).get(
                        "count", 0
                    )
                    if video_uploads > 0:
            ...
            with col2:
                total_operations = sum(
                    data["count"] for data in operation_counts.values()
                )
    ...
    cost_data = [
        (
            "📹 Video Upload",
            "5 credits",
            "Full video analysis with captions, blog, social post, shorts, and thumbnails",
        ),
        ...
        (
            "✨ Tweet Enhancement",
            "1 credit",
            "Enhanced social post with AI optimization",
        ),
    ]
...
try:
    hf_client = InferenceClient(
        "stabilityai/stable-diffusion-xl-base-1.0", token=HF_TOKEN
    )
...
def clip_video_ffmpeg(video_path, start_time_str, end_time_str):
    ...
    try:
        ...
        st.error(
            f"FFmpeg clipping failed. Make sure ffmpeg is installed. Error: {e.stderr.decode()}"
        )
        return None
    except Exception as e:
...
def process_video_with_progress(
    uploaded_file, video_path, target_platform="General", enable_grounding=True
):
    """
...
"""
            ...
            status_text.text(
                f"🤖 Analyzing video with {provider.upper() if video_file else 'fallback'}..."
            )
            ...
            results = llm_client.analyze_video(
                video_file, analysis_prompt, enable_grounding=enable_grounding
            )
...
def process_video_with_llm(
    video_path, target_platform="General", enable_grounding=True
):
    """
...
"""
    ...
    with st.spinner(
        f"🤖 Analyzing video with {provider.upper()} for {target_platform}..."
    ):
        results = llm_client.analyze_video(
            video_file, analysis_prompt, enable_grounding=enable_grounding
        )
...
def home_page():
    ...
    st.info(
        "🔍 **New Feature**: All generated content is now verified against the video transcript to prevent AI hallucinations!"
    )
    st.success(
        "💾 **Persistent Storage**: All your videos and content are automatically saved to a local database for easy browsing and reuse!"
    )
...
def creator_tool_page():
    ...
    uploaded_file = st.file_uploader(
        "Upload your video file", type=["mp4", "mov", "webm", "mkv"]
    )
    ...
    if uploaded_file:
        ...
        if st.button(
            "🚀 Analyze Video & Generate All Content",
            type="primary",
            use_container_width=True,
        ):
            # Check if user has enough credits
            has_credits, balance, cost = credits_manager.has_sufficient_credits(
                "video_upload"
            )

            if not has_credits:
                st.error(
                    f"❌ Insufficient credits! You need {cost} credits but only have {balance}."
                )
            ...
            st.info(
                f"💳 This operation will cost **{cost} credits**. Current balance: {balance}"
            )

            # Deduct credits
            success, new_balance = credits_manager.deduct_credits(
                "video_upload", description=f"Video analysis: {uploaded_file.name}"
            )
            ...
            if st.session_state.results and "error" not in st.session_state.results:
                st.success(
                    f"✅ Full analysis complete for {st.session_state.selected_platform}!"
                )
                ...
                st.warning(
                    "⚠️ Using fallback/mock results (primary AI provider unavailable)"
                )
            else:
    ...
    if st.session_state.results and st.session_state.results.get("captions"):
        ...
        with status_col1:
            st.success(
                f"📊 Content optimized for: **{st.session_state.selected_platform}**"
            )
        with status_col2:
        ...
        with tabs[1]:
            ...
            for i, short in enumerate(shorts):
                with st.container(border=True):
                    ...
                    st.markdown(
                        f"**Timestamps:** `{short.get('start_time', 'N/A')} - {short.get('end_time', 'N/A')}`"
                    )
                    ...
                    if st.button(f"✂️ Prepare Clip {i+1}", key=f"clip_{i}"):
                        has_credits, balance, cost = (
                            credits_manager.has_sufficient_credits("shorts_clip")
                        )
                            ...
                            with st.spinner("Clipping with ffmpeg..."):
                                ...
                                if clip_path:
                                    ...
                                    credits_manager.deduct_credits(
                                        "shorts_clip", description=f"Shorts clip {i+1}"
                                    )
        ...
        with tabs[2]:
            ...
            if "blog_post_original" in results:
                compare_tab1, compare_tab2 = st.tabs(
                    ["✅ Verified Version", "⚠️ Original (Unfiltered)"]
                )
                with compare_tab1:
...
with st.sidebar:
    ...
    page = st.radio(
        "Navigation",
        ["Home", "Creator Tool", "History", "Credits"],
